  ...
  async (req, res) => {
    try {
      ...
      const topics = JSON.parse(
        req.body.topics || '["Skills", "Projects", "Experience"]',
      );
      ...
      if (!resumeText || resumeText.trim().length < 50) {
        return res
          .status(400)
          .json({ error: "Resume content is too short or empty" });
      }
      ...
      const topicFocus = topics
        .map((topic) => {
          const topicMap = {
            ...
            Education:
              "educational background, degrees, and academic achievements",
            Certifications:
              "certifications, licenses, and professional credentials",
            Achievements: "accomplishments, awards, and notable achievements",
            Activities:
              "extracurricular activities, volunteer work, and personal interests",
          };
...
Focus on ${topicFocus} as mentioned in the resume. Ensure questions match the ${difficultyDescriptions[difficultyLevel]} difficulty level.`;
      ...
      for (let line of lines) {
        ...
        if (cleaned.length > 10) {
          // Only add substantial questions
      ...
      if (finalQuestions.length < Math.ceil(numberOfQuestions / 2)) {
        return res
          .status(500)
          .json({
            error: `Failed to generate sufficient questions. Got ${finalQuestions.length} but needed at least ${Math.ceil(numberOfQuestions / 2)}`,
          });
      }
